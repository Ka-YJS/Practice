...
7. 숫자 데이터 타입 종류와 특징
    ...
    |BINARY_DOUBLE|64비트 부동소수점 수|최대 4byte|
    |TINYINT|가장작은 숫자자료형/부호가 있으면 -128~127 / 부호가 없으면 0~255|1byte|
    |SMALLINT|부호가 있으면 -32768~32767 / 부호가 없으면 0~65535|2byte|
    |MIDIUMINT|부호가 있으면 -8388608~8388607 / 부호가 없으면 0~16777215|3byte|
    |INT|일반적인 숫자자료형 / 부호가 있으면 -2147483648~2147483647 / 부호가 없으면 0~4294967295|4byte|
    |INTEGER|INT와 표기법만 다르고 동일함|4byte|
    |BIGINT|부호가 있으면 -922337036854775808~922337036854775807 / 부호가 없으면 0~18446744073709551615|8byte|
    |DOUBLE|FLOAT보다 더 큰 범위를 가짐|8byte|
    |DECIMAL|위/경도데이터를 사용할때만 사용됨 / 소수를 저장하는 용도로 사용 / 내부적으로는 문자형태로 저장되는 타입||
