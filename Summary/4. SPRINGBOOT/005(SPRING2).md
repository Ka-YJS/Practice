...
-이를 통해 캐시 제어, 인증 정보, 콘텐츠 타입 등을 제어할 수 있음
  - 응답 본문 제어
-sponseEntity는 응답 본문(body)에 객체나 JSON 데이터를 포함할 수 있음
...
4sp. nseEntity.badRequest(): 400 Bad Request 응답을 반환하는 메서드
5>ResponseEntity.notFound(): 404 Not Found 응답을 반환하는 메서드
-. 우리가 작성할 컨트롤러는 모두 ResponseEntity를 반환할 예정임

5상적으로 응답을 반환한다면 return ResponseEntity.ok().body(response); 라고 뜸

. 1.  서비스 레이어

1)서비스레이어는 컨트롤러와 퍼시스턴스(영속) 사이에서 비즈니스 로직(서비스를 할 때 필요한 로직)을 수행하는 역할을 함

HTTP와 긴히 연결된 컨느롤러에서 분리돼 있고, 또 데이터베이스와 긴밀히 연관된 퍼시스턴스와도 분리되어 있. 
...
4)@Service
  ...
  - @Component 어노테이션이 붙은 클래스는 Bean으로 만들어주는데, @Service는 @Componen어노테이션의 자식개념으로
볼 수 있
4>스프링이 @S. rvice도 찾아서 Bean으로 만들어줌
...
-즉, 스프링은 애플리케이션이 실행될 때 이 클래스의 객체를 생성하고 관리하며, 다른 컴포넌트에서 필요할 때 자동으로 주입해줌
  - 계층 구조의 명확성
-ervice는 계층화된 아키텍처에서 서비스 레이어의 역할을 명확하게 구분하는 데 도움을 줌
-이를통해 각 레이어의 책임을 분리하고, 코드의 가독성과 유지보수성을 향상시킴
-스프. 은 @Service, @Repository, @Controller 등을 통해 각 클래스가 어떤 역할을 하는지 직관적으로 알 수 있게 해줌


. 2. 퍼시스턴스 레이어 : 스프링 데이터 JPA_관계형 데이터베이스를 자바와 연결하려면 어떻게 해야하는가?

1)DBMS를 설치

테이블을 생성하고 아이템넣기
. )테이블을 생성하는 것도, 테이블에 엔트리를 추가하는것도, 수정, 삭제하는것도 다 웹 서비스의 일부로 동작해야 함
...
1)메모리 기반과 디스크 기반 지원
  - 메모리 기반 데이터베이스: 애플리케이션이 실행되는 동안에만 데이터베이스가 유지되며, 
...
5)간편한 설정 및 사용
  ...
  - 웹 기반의 H2 콘솔(http://localhost:9090/h2-console에 접속하면, SQL 쿼리를 직접 실행하거나 데이터를 조회할 수 있음)을 제공하여, 
...
6)쿼리 메서드(Query Method)
  ...
  - 예를 들어, findByUsername(String username) 메서드는 자동으로 SELECT * FROM User WHEREsername = ? 쿼리를 실행함
...
2객체.  계속 만드는 것이 아닌, 한번 만들어진 객체를 계속 하는 것 -> 싱글톤 패턴
...
서스 개발 실습
  - P. ST로 요청했을 때 CREATE PEST API 개발
  - GET으로 요청했을 때 RETRIEVE(검색) REST I 개발
  - UPDATE로 요청했을 때 UPDATE REST API 개
4ELETE로 요청했을 때 DELETE REST API 개발6). . 계층 -> 서비스계층 -> 표현계층 순으로 구. 
