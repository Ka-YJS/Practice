...
const Map = () => {
  ...
  const GOOGLE_API_KEY = "GOOGLE_API_KEY";//여기에 실제 API 키 입력
  ...
  const handleSavePost = async () => {
   //필수 입력값 확인
    ...
    try {
     ...
     //Send POST request
      const response = await axios.post(`백엔드엔드포인트/api/write/${user.id}`, formData, {
        headers: {
...
mapRef.current.animateToRegion(regionRef.current, 1000);
```
1. 지도를 새로운 위치로 1초(1000ms) 동안 부드럽게 이동시키는 애니메이션 함수임
2. regionRef.current에 저장된 좌표로 이동
```JS
<KeyboardAvoidingView
  style={styles.container}
  behavior={Platform.OS === "ios" ? "padding" : "height"}>
```
1. KeyboardAvoidingView : 키보드가 입력창을 가리지 않도록 자동으로 컨텐츠 위치를 조정해줌
2. behavior의 기능
  - "padding" : iOS용, 키보드 위에 패딩 추가
  - "height" : 안드로이드용, 키보드 높이만큼 뷰 높이 감소
```JS
<FlatList
  ListHeaderComponent={ <> ... }
  ListFooterComponent={ <>... }
/>
```
1. FlatList : 보이는 항목만 렌더링하는 효율적인 스크롤 목록
2. ListHeaderComponent : 목록 상단 고정 컨텐츠(지도, 검색 기능)
3. ListFooterComponent : 목록 하단 고정 컨텐츠(장소 목록, 글쓰기 폼)
4. <>(Fragment)의 기능
  - <Fragment\>와 같음 -> 동일한 기능을 함
  - 불필요한 div나 View 같은 추가 노드 없이 여러 요소를 그룹화할 때 사용함
  - 여기서는 추가 View 없이 여러 요소를 그룹화함